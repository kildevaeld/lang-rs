...
mod tokens {
    ...
    {
        fn peek(cursor: &mut lang::parsing::TokenReader<'input, '_, T>) -> bool {
...
mod exprs {
    ...
    for BinaryOperator {
        fn parse(
            ...
            Err(lang::parsing::Error::new(("BinaryOperator", errors), span))
        }
    }
    ...
    for BinaryOperator {
        fn peek(
            cursor: &mut lang::parsing::TokenReader<
                'parse,
                '_,
                lang::lexing::tokens::Token<'parse>,
            >,
    ...
    impl<'a> lang::lexing::WithSpan for BinaryExpr<'a> {
        fn span(&self) -> lang::lexing::Span {
            self.left.span() + self.operator.span() + self.right.span()
        }
    }
    impl<'parse: 'a, 'a> lang::parsing::Peek<'parse, lang::lexing::tokens::Token<'parse>>
    for BinaryExpr<'a> {
    ...
    impl<'a> lang::lexing::WithSpan for CallExpr<'a> {
        fn span(&self) -> lang::lexing::Span {
            self.target.span() + self.args.span()
        }
    }
    impl<'parse: 'a, 'a> lang::parsing::Peek<'parse, lang::lexing::tokens::Token<'parse>>
    for CallExpr<'a> {
    ...
    impl<'a> Expr<'a> {
        pub fn accept_mut<V: ExprVisitorMut<'a>>(
            ...
            match self {
                Self::Literal(field_0) => visitor.visit_mut_literal_expr(field_0),
                Self::Ident(field_0) => visitor.visit_mut_ident_expr(field_0),
    ...
    mod expression {
        ...
        fn __get_precedence<'input>(
            ...
            if input.peek::<BinaryOperator>() {
                1u8
            } else if input.peek::<Args>() {
                2u8
            } else {
                0u8
            }
        ...
        fn __prefix<'input>(
            ...
            if input.peek::<Literal>() {
                ...
                { Ok(Expr::Ident(IdentExpr { ident: i })) }
            } else {
...
mod stmt {
    ...
    impl<'a> lang::lexing::WithSpan for FuncStmt<'a> {
        fn span(&self) -> lang::lexing::Span {
            self.fn_token.span() + self.name.span() + self.params.span()
                + self.body.span()
        }
    }
    impl<'parse: 'a, 'a> lang::parsing::Peek<'parse, lang::lexing::tokens::Token<'parse>>
    ...
    impl<'a> lang::lexing::WithSpan for ForStmt<'a> {
        fn span(&self) -> lang::lexing::Span {
            self.for_token.span() + self.name.span() + self.in_token.span()
                + self.body.span()
        }
    }
    impl<'parse: 'a, 'a> lang::parsing::Peek<'parse, lang::lexing::tokens::Token<'parse>>
    ...
    impl<'a> lang::lexing::WithSpan for IfStmt<'a> {
        fn span(&self) -> lang::lexing::Span {
            self.for_token.span() + self.name.span() + self.body.span()
        }
    }
    impl<'parse: 'a, 'a> lang::parsing::Peek<'parse, lang::lexing::tokens::Token<'parse>>
    for IfStmt<'a> {
    ...
    impl<'a> lang::lexing::WithSpan for LetStmt<'a> {
        fn span(&self) -> lang::lexing::Span {
            self.let_token.span() + self.name.span() + self.assign_token.span()
                + self.value.span()
        }
    }
    impl<'parse: 'a, 'a> lang::parsing::Peek<'parse, lang::lexing::tokens::Token<'parse>>
    ...
    impl<'a> lang::lexing::WithSpan for ReturnStmt<'a> {
        fn span(&self) -> lang::lexing::Span {
            self.return_token.span() + self.expr.span()
        }
    }
    impl<'parse: 'a, 'a> lang::parsing::Peek<'parse, lang::lexing::tokens::Token<'parse>>
    for ReturnStmt<'a> {
    ...
    > lang::parsing::Parse<'parse, lang::lexing::tokens::Token<'parse>> for Stmt<'a> {
        fn parse(
            ...
            Err(lang::parsing::Error::new(("Stmt", errors), span))
        }
    }
...
fn main() {
    ...
    let mut parser = Parser::from_tokens(input, lexer.skip_whitespace(false).tokenize())
        .expect("lex");
